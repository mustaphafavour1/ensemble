...
export function HexWorldMap<T extends HexWorldMapPoint>({
  ...
  const callouts = useMemo(() => {
    return [...points]
      ...
      .map((point) => {
        ...
        const leftPct = (x / LAND.viewBoxWidth) * 100;
        const topPct = (y / LAND.viewBoxHeight) * 100;
        const maxValue = Math.max(...points.map((p) => p.value), 1);
        const t = point.value / maxValue;
        return { point, leftPct, topPct, tone: toneFor(t), ...formatCallout(point) };
      });
  ...
  return (
    <div className="relative size-full">
      <svg viewBox={`0 0 ${LAND.viewBoxWidth} ${LAND.viewBoxHeight}`} className="size-full" preserveAspectRatio="xMidYMid meet">
        {hexCells.map((cell, i) => (
          <polygon
            key={i}
            points={cell.poly}
            ...
            onMouseEnter={cell.pointId ? () => setHoveredId(cell.pointId) : undefined}
            onMouseLeave={cell.pointId ? () => setHoveredId(null) : undefined}
          />
        ))}
      </svg>

      <div className="pointer-events-none absolute inset-0">
        {callouts.map(({ point, leftPct, topPct, tone, title, metric }) => (
          <div key={point.id}>
            <div
        ...
        {hovered && hoveredPos && hoveredTooltip && (
          <div
            ...
            style={{
              left: `${(hoveredPos.x / LAND.viewBoxWidth) * 100}%`,
              top: `${(hoveredPos.y / LAND.viewBoxHeight) * 100}%`,
              transform: `translate(-50%, ${(hoveredPos.y / LAND.viewBoxHeight) * 100 < 22 ? "12px" : "calc(-100% - 12px)"})`,
            }}
          >
