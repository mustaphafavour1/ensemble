...
export function NewAgentDialog({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast.success(`${name || "New agent"} created`, {
      description: "It will appear in the Code Agent Fleet once demo data refreshes.",
    });
    onOpenChange(false);
    setName("");
    setScope("");
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogTitle className="mt-2">New Agent</DialogTitle>
          <DialogDescription>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
            ...
            <div className="flex flex-col gap-3 rounded-md border border-border p-3">
              {TRIGGERS.map((t) => (
                <div key={t.key} className="flex items-center justify-between gap-4">
                  <div>
                    <p className="text-xs text-ink-em">{t.label}</p>
                    <p className="text-2xs text-ink-faint">{t.hint}</p>
                  </div>
                  <Switch
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <div className="relative">
              <span className="pointer-events-none absolute top-1/2 left-3 -translate-y-1/2 text-xs text-ink-faint">
                $
              </span>
              <Input
                id="agent-budget"
