...
export function NewEnvironmentDialog({
  ...
  const [template, setTemplate] = useState(defaultTemplateId ?? STACK_TEMPLATES[0].id);
  const [region, setRegion] = useState(REGIONS[0]);
  const [size, setSize] = useState(SIZES[1].value);
  const [repo, setRepo] = useState(REPOS[0].id);

  // The dialog can be opened from several "Use template" triggers, each
  // wanting a different starting template — resync whenever it transitions
  // to open, adjusting state during render (React's documented pattern)
  // rather than an effect, so it settles before paint.
  const [wasOpen, setWasOpen] = useState(open);
  if (open !== wasOpen) {
    setWasOpen(open);
    if (open) setTemplate(defaultTemplateId ?? STACK_TEMPLATES[0].id);
  }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast.success(`Environment queued for ${repoName}`, {
      description: "It will appear in the Sandbox Playground once provisioning starts.",
    });
    onOpenChange(false);
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogTitle className="mt-2">New Environment</DialogTitle>
          <DialogDescription>Spin up an environment from a template.</DialogDescription>
        </DialogHeader>
