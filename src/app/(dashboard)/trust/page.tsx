...
export default function ProvenanceTrustPage() {
                ...
                {pageItems.map((event) => {
                  ...
                  return (
                    <li key={event.id} className="relative flex gap-4">
                      <div className="relative z-10 flex size-8 shrink-0 items-center justify-center rounded-full border border-border bg-canvas text-ink-300">
                        <Icon className="size-3.5" strokeWidth={1.75} />
                      </div>
                      <div className="flex-1 pt-1 pb-1">
                        <p className="text-xs text-ink-100">{messageFor(event)}</p>
                        <div className="mt-1.5 flex flex-wrap items-center gap-x-3 gap-y-1 text-2xs text-ink-500">
