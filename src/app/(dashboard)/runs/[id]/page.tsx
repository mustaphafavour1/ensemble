...
export default function RunDetailPage() {
  ...
  if (!run) {
    return (
      <div className="flex flex-col items-center justify-center py-24 text-center">
        <p className="text-sm text-ink-100">Run not found</p>
  ...
  return (
    <div>
      <Link
        href="/runs"
        className="mb-4 inline-flex items-center gap-1.5 text-2xs text-ink-300 hover:text-ink-100"
      >
      ...
      <div className="mb-6 flex flex-wrap items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <span className="font-mono text-2xs text-ink-500">{run.id}</span>
          </div>
          <h1 className="mt-2 font-mono text-xl text-ink-100">{run.title}</h1>
          <div className="mt-2 flex flex-wrap items-center gap-x-4 gap-y-1.5 text-xs text-ink-300">
            <AgentTag name={agent.name} />
            <span className="font-mono text-ink-500">{agent.model.name}</span>
            <span className="font-mono">{repo.name}</span>
            <StackTag stack={stack} />
            <span className="font-mono text-2xs text-ink-500">{run.branch}</span>
          </div>
        </div>
        <div className="flex gap-4 text-right">
          <div>
            <p className="text-2xs text-ink-500">Duration</p>
            <p className="font-mono text-sm text-ink-100 tabular-nums">
              {run.durationMs ? formatDuration(run.durationMs) : "—"}
            </p>
          </div>
          <div>
            <p className="text-2xs text-ink-500">Cost</p>
            <p className="font-mono text-sm text-ink-100 tabular-nums">
      ...
      <div className="grid grid-cols-3 gap-4">
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                <FileCode2 className="size-3.5 text-ink-500" />
            ...
            <CardContent>
              <div className="max-h-64 overflow-y-auto rounded-md bg-neutral-950 p-3">
                {detail.logLines.map((line, i) => (
                  <p
                    key={i}
                    className="font-mono text-2xs leading-relaxed text-ink-300"
                  >
        ...
        <div className="col-span-1 flex flex-col gap-4">
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                <ShieldCheck className="size-3.5 text-ink-500" />
            ...
            <CardContent>
              ...
              <dl className="mt-4 flex flex-col gap-2.5 border-t border-border pt-3 text-2xs">
                <div className="flex items-center justify-between">
                  ...
                  <dd className="font-mono text-ink-100">{run.commitSha}</dd>
                </div>
                <div className="flex items-center justify-between">
                  <dt className="text-ink-500">Reviewer</dt>
                  <dd className="text-ink-100">{run.reviewer ?? "Unreviewed"}</dd>
                </div>
                <div className="flex items-center justify-between">
                  <dt className="text-ink-500">Started</dt>
                  <dd className="font-mono text-ink-100">{formatDate(run.startedAt)}</dd>
                </div>
              </dl>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center gap-2">
                <Receipt className="size-3.5 text-ink-500" />
            ...
            <CardContent>
              <dl className="flex flex-col gap-2.5 text-2xs">
                <div className="flex items-center justify-between">
                  <dt className="text-ink-500">Input tokens</dt>
                ...
                <div className="flex items-center justify-between border-t border-border pt-2.5">
                  <dt className="text-ink-500">Input cost</dt>
                  <dd className="font-mono text-ink-100 tabular-nums">
                    ${detail.cost.inputCost.toFixed(2)}
                  </dd>
                </div>
                <div className="flex items-center justify-between">
                  <dt className="text-ink-500">Output cost</dt>
                  <dd className="font-mono text-ink-100 tabular-nums">
                    ${detail.cost.outputCost.toFixed(2)}
                  </dd>
                </div>
                <div className="flex items-center justify-between border-t border-border pt-2.5">
                  <dt className="font-medium text-ink-100">Total</dt>
